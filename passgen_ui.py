...
class View:
    def __init__(self, main):
        ...
        self.length_display = CTkEntry(
            self.main, textvariable=self.model.length, width=35)
        self.length_slider = CTkSlider(
            self.main, variable=self.model.length, from_=4, to=100, width=20, orientation='vertical')

        # password options checkboxes
        self.uppercase_checkbutton = CTkCheckBox(
            self.main, text='Uppercase', variable=self.model.uppercase)
        self.lowercase_checkbutton = CTkCheckBox(
            self.main, text='Lowercase', variable=self.model.lowercase)
        self.numbers_checkbutton = CTkCheckBox(
            self.main, text='Numbers', variable=self.model.numbers)
        self.special_checkbutton = CTkCheckBox(
            self.main, text='Special Characters', variable=self.model.special_characters)

        # buttons
        self.generate_button = CTkButton(
            self.main, text='Generate', fg_color='green', hover_color='darkgreen')
        self.copy_button = CTkButton(self.main, text='Copy')
        self.exit_button = CTkButton(
            self.main, text='Exit', fg_color='red', hover_color='darkred')

        # password generator
        self.password_entry = CTkEntry(self.main, textvariable=self.model.password, width=715,
                                       state='readonly', font=CTkFont(size=12, family='Consoleas'), justify='center')
